...
class HeadersModule(Module[HeaderAnalysis]):
    ...
    def analyze(self) -> HeaderAnalysis:
        ...
        if "Content-Security-Policy" not in present:
            flags.append("Content-Security-Policy is not present.")
            explanations.append(
                "CSP can reduce the impact of certain classes of browser-side injection."
            )

        if "Strict-Transport-Security" not in present:
            flags.append("Strict-Transport-Security is not present.")
            explanations.append(
                "HSTS tells compatible browsers to prefer HTTPS for the site."
            )

        if "X-Content-Type-Options" not in present:
            flags.append("X-Content-Type-Options is not present.")

        if "X-Frame-Options" not in present:
            flags.append("X-Frame-Options is not present.")

        if "Server" in informational:
            flags.append("Server response header is exposed.")

        if "X-Powered-By" in informational:
            flags.append("X-Powered-By response header is exposed.")
